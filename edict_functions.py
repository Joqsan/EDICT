...
def img2img_editing(
    im_path, edit_prompt, steps=50, init_image_strength=0.7, guidance_scale=3
):
    """
    ...
    return baseline_stablediffusion(
        edit_prompt,
        init_image_strength=init_image_strength,
        steps=steps,
        init_image=orig_im,
        ...
        for i in range(hidden_states.shape[0] // slice_size):
            ...
            if self.use_last_attn_slice:
                if self.last_attn_slice_mask is not None:
                    ...
                    attn_slice = (
                        attn_slice * (1 - self.last_attn_slice_mask)
...
def baseline_stablediffusion(
    ...
    if prev_image is not None:
        prev_scheduler = LMSDiscreteScheduler(
            beta_start=0.00085,
            beta_end=0.012,
            beta_schedule="scaled_linear",
    ...
    if fixed_starting_latent is None:
        noise = torch.randn(
            init_latent.shape, generator=generator, device=device, dtype=unet.dtype
        )
        if scheduler_str == "ddim":
            if init_image is not None:
    ...
    if prev_image is not None:
        ...
        with autocast(device):
            prev_init_latent = (
                vae.encode(prev_image).latent_dist.sample(generator=generator) * 0.18215
            )

        t_start = steps - int(steps * init_image_strength)

        prev_latent = prev_scheduler.add_noise(prev_init_latent, noise, t_start).to(
            device
        )
    else:
        prev_latent = None
    ...
    with autocast(device):
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            ...
            if prev_latent is not None:
                ...
                prev_latent_model_input = (
                    prev_latent_model_input / ((sigma**2 + 1) ** 0.5)
                ).to(unet.dtype)
                prev_noise_pred_uncond = unet(
                    prev_latent_model_input,
                    t,
                    encoder_hidden_states=embedding_unconditional,
                ).sample
            ...
            if prompt_edit is not None or prev_latent is not None:
                if (
                    t_scale >= prompt_edit_tokens_start
                    and t_scale <= prompt_edit_tokens_end
                ):
                    use_last_tokens_attention()
                if (
                    t_scale >= prompt_edit_spatial_start
            ...
            if use_cond_guidance:
                ...
                noise_pred = noise_pred_uncond
            if (
                clip_guidance is not None
                and t_scale >= clip_start
                and t_scale <= clip_end
            ):
                noise_pred, latent = new_cond_fn(
                    latent,
                    t,
                    t_index,
            ...
            if scheduler_str == "ddim":
                latent = forward_step(scheduler, noise_pred, t, latent).prev_sample
            else:
            ...
            if prev_latent is not None:
                ...
                prev_latent = prev_scheduler.step(
                    prev_noise_pred, t_index, prev_latent
                ).prev_sample
            if one_pass:
                break
...
def coupled_stablediffusion(
    ...
    ## If running EDICT then no leap steps
    if run_baseline:
